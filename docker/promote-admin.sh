...
#
# This script promotes an existing user to admin role in the Infrss instance.
#
# Usage:
...
USER_ID=$(docker compose -f "$SCRIPT_DIR/supabase/docker-compose.yml" --project-name infrss exec -T db psql -U postgres -d postgres -t -c \
...
docker compose -f "$SCRIPT_DIR/supabase/docker-compose.yml" --project-name infrss exec -T db psql -U postgres -d postgres -c \
